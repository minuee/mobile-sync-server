...
python3 scripts/append_experiment_ledger.py \
  ...
  --run-id "$RUN_ID" \
  --date "$TODAY" \
  --key-outcome "controlled-device run finalized" \
  --notes "auto-appended by finalize_controlled_device_run.sh"

printf 'finalized controlled-device run into %s and appended ledger\n' "$EVIDENCE_ROOT"
